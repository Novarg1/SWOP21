...
public class Scheduler implements Observer {
	...
	public void update(Observable obs, Object obj) {
		...
		for (Assemblyline al : assemblylines) {
			Order next = algorithm.getNextFor(al, pending, this.getDay()); //TODO this.getDay toegevoegd
			if (this.getETA(next, this.getDay()).getDay() == al.getCurrentTime().getDay()) { //this.getDay() toegevoegd
	...
	private boolean shouldStartNewDay() {
		...
		Map<Assemblyline, List<Order>> schedule = algorithm.schedule(pending, this.getDay());//TODO this.getDay toegevoegd
		...
		for (Order next : nextOrders) {
			int etaDay = algorithm.getETA(next, pending,this.getDay()).getDay(); //TODO this.getDay toegevoegd
	 ...
	 *         schedule.
	 */
	public Timestamp getETA(Order order, int day) { //int day toegevoegd
		return algorithm.getETA(order, this.pending, day);
	}
	 ...
	 */
	public int getDay() {
		return ((Assemblyline) this.assemblylines.toArray()[0])
