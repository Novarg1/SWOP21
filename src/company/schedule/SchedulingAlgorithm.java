...
public abstract class SchedulingAlgorithm {
	 ...
	 */
	private Map<Assemblyline, Assemblyline> originalAssemblylines = new HashMap<>(); //TODO mocht dit?
	/**
	 ...
	 *         as values.
	 */
	public Map<Assemblyline, List<Order>> schedule(List<Order> pending, int day) {
		...
		// first schedule the orders that wouldn't meet their deadline otherwise
		this.handleDeadlines(day);
	 ...
	 *         such order.
	 */
	public Order getNextFor(Assemblyline ass, List<Order> pending,int day) { //TODO int day toegevoegd
		List<Order> list = this.schedule(pending,day).get(ass); //TODO day toegevoegd
	 ...
	 * @return
	 */
	public Timestamp getETA(Order order, List<Order> pending,int day) {
		this.schedule(pending,day);
		if(!pending.contains(order))
			throw new IllegalArgumentException("This order is not in the schedule");
		if(etas.get(order) != null)//
			return etas.get(order);
		else{//
			return Timestamp.beginningOfDay(10);//
		}//TODO lijnen met // achter moeten weg
	}
	 ...
	 * immediately.
	 */
	private void handleDeadlines(int day) {
...
//		int day = ((Assemblyline) this.schedule.keySet().toArray()[0]).getCurrentTime().getDay();
//		for (; !list.isEmpty(); day++) {}
	...
	private void initialize(List<Order> pending) {
		if(originalAssemblylines == null)
			System.out.println("originalAsemblylines is null");
