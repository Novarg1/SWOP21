...
public class StatisticsController {
	...
	/**
	 * takes a system controller as parameter and uses this to compute all
	 * statistics. If things change after the creation of this controller it has
	 * to be reinitialised
	 * 
	 * @param s
	 */
	public StatisticsController(SystemController s) {
		...
		int nDays = systemController.getScheduler().getDay();

		averageCarsPerDay = finishedOrders.size() / nDays;
		medianCarsPerDay = (int) (finishedOrders.size() / nDays);

		int yesterday = systemController.getScheduler().getDay() - 1;
